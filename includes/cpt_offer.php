...
function dob_cmb_vote_html( $post ) {
	...
	$dob_cmb_vote = get_post_meta( $post->ID, 'dob_cmb_vote', true );
...
(function($) {/*{{{*/
  ...
  $(function() {
		...
		$('#btn_dob_vm_add').click( function() {
			...
			if ( ol.find('li').length > 8 ) {
				alert('Check Count (MAX:8)');
				return;
			}
			...
			<thead>
				<tr>
					...
					<th>
						...
						<input type="button" id="btn_dob_vm_add" class="button" value="<?php echo $label_add;?> (MAX:8)" style="display:<?php echo ($dob_vm_type=='updown')?'none':'inline';?>;">
					</th>
				</tr>
			</thead>
			<tbody>
...
function dob_save_cmb_data( $post_id ) {
	...
	$dob_cmb_vote = array( 'type' => $_POST['dob_vm_type'] );
	if ( isset($_POST['dob_vm_data']) && is_array($_POST['dob_vm_data']) ) {
		$dob_cmb_vote['data'] = $_POST['dob_vm_data'];	// choice, plural
	}
	update_post_meta( $post_id, 'dob_cmb_vote', $dob_cmb_vote );
}
